...
export default function PunchingClock() {
  ...
  // Determine the entry type (Entry or Exit) based on previous records
  const determineEntryType = async (barcode) => {
    try {
      // Fetch the latest entry for this barcode
      const response = await fetch(`/api/barcode-entries/last/${barcode}`);

      // Process the response even if it's 404 (no previous entries)
      if (response.status === 404) {
        console.log(
          `No previous entries found for barcode ${barcode}, defaulting to Entrada`
        );
        return 'Entrada';
      }
      ...
      if (!response.ok) {
        ...
        console.log(
          `API responded with status: ${response.status}, defaulting to Entrada`
        );
        return 'Entrada'; // Default to entry if we can't determine
      }

      const data = await response.json();

      // Add additional logging to help diagnose issues
      console.log(`Previous entry data for ${barcode}:`, data);

      // If no previous entries or the last action was "Salida", this should be an entry
      if (!data || !data.entry || data.entry.action === 'Salida') {
        return 'Entrada';
      }

      // Otherwise, this should be an exit
      return 'Salida';
    } catch (error) {
      // Suppress error message in console but log it more clearly
      console.log(
        'Error while determining entry type (defaulting to Entrada):',
        error.message
      );
      return 'Entrada'; // Default to entry on error
    }
  };
    ...
    if (codeToSave && codeToSave.trim() !== '') {
      ...
      try {
        if (photoData || !isRecording) {
          ...
          const actionType = await determineEntryType(codeToSave);

          const newEntry = {
            barcode: codeToSave,
            timestamp,
            photo: photoData,
            action: actionType, // Add the action type (Entrada/Salida)
          };
          ...
          if (result.success) {
            ...
            if (notificationRef.current) {
              ...
              notificationRef.current.showSuccess({
                ...
                name: employeeName,
                photo: photoData,
                action: actionType, // Pass the action type to the notification
              });

              // Speak the action type
              speakText(`${actionType} registrada exitosamente`);
            }
      ...
      // Clear any pending setTimeout to prevent duplicate submissions
