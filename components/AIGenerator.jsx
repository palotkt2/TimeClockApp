...
import { generateTemplate, generateImage } from '../services/aiTemplateService';
  ...
  const handleGenerateTemplate = async () => {
    ...
    try {
      ...
      if (generationType === 'template') {
        ...
        updateToast(
          toastId,
          'Generando fondo de imagen con OpenAI DALL-E...',
          'info'
        );
        try {
          ...
          if (imageResponse && imageResponse.imageUrl) {
            ...
            const sourceMessage =
              imageResponse.source === 'dall-e'
                ? 'Fondo de imagen creado exitosamente con OpenAI DALL-E!'
                : `Fondo de imagen generado usando ${imageResponse.source} (OpenAI DALL-E no disponible)`;

            updateToast(toastId, sourceMessage, 'success');
          } else {
            updateToast(
              toastId,
              'Error al generar la imagen. Intente de nuevo.',
              'error'
            );
          }
        } catch (imageError) {
          ...
          updateToast(
            toastId,
            `Error al generar imagen: ${imageError.message || 'Error de API'}`,
            'error'
          );
        }
      } else {
      ...
      showToast(
        `Error al generar ${generationType}: ${
      ...
      <div className="space-y-3">
          ...
          <div className="bg-blue-50 p-3 rounded-md border border-blue-100">
            ...
            {lastResponse?.enhancedPrompt && (
              <p className="text-xs text-blue-700 mt-2">
                <strong>Prompt utilizado:</strong> {lastResponse.enhancedPrompt}
              </p>
            )}
          </div>
        )}
        ...
        {generationType === 'picture' && (
          <div className="text-xs text-gray-500 mt-2">
            <p>
              Las imágenes generadas están optimizadas para ser usadas como{' '}
              <strong>fondos</strong> de credenciales. Para mejores resultados:
            </p>
            <ul className="list-disc ml-4 mt-1 space-y-1">
              <li>Describe colores, patrones o estilos</li>
              <li>Evita mencionar textos, logotipos o formas complejas</li>
              <li>Sé específico con el estilo: abstracto, moderno, etc.</li>
            </ul>
          </div>
        )}
      </div>
